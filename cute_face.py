...
class CuteRenderer:
    ...
    def __init__(self, screen):
        ...
        self.face_center_y = HEIGHT // 2 - 30
        # 眼睛尺寸 (黑色竖椭圆，无眼白)
        self.eye_rx = 32            # 眼 X 半径
        self.eye_ry = 40            # 眼 Y 半径
        self.eye_spacing = 66       # 眼间距(中心到中心的 X 偏移)
        self.eye_y_offset = -15     # 眼睛 Y 偏移(相对 face_center)
        # 眉毛
        self.brow_y_offset = -85    # 眉基 Y 偏移(远离眼球)
    ...
    def _draw_blush(self, cx, cy, blush_val, scale):
        ...
        if alpha < 5: return
        for side in [-1, 1]:
            bx = cx + side * (self.eye_spacing + 58) * scale
            by = cy + (self.eye_y_offset + 55) * scale
            brx = int(28 * scale); bry = int(16 * scale)
    ...
    def _draw_eyebrow(self, cx, cy, brow_offset, face_scale, side):
        """渐细圆润眉毛：重叠圆点串，外粗→内细。
        左边: 左上↘右下  右边: 右上↙左下"""
        blen = int(55 * face_scale)
        thick_outer = max(3, int(5 * face_scale))
        thick_inner = max(1, int(2 * face_scale))

        tilt = brow_offset * face_scale * 1.5
        half = blen // 2

        if side < 0:
            x1, y1 = cx - half, cy - int(8 * face_scale) + tilt
            x2, y2 = cx + half, cy + int(2 * face_scale) - tilt * 0.5
        else:
            x1, y1 = cx + half, cy - int(8 * face_scale) + tilt
            x2, y2 = cx - half, cy + int(2 * face_scale) - tilt * 0.5
